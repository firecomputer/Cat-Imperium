...
static func summarize(provinces: Array[Province], land: PackedByteArray,
	...
	# 섬(컴포넌트 ≤ 30)이 정확히 1개의 프로빈스인지
	var island_comp_total := 0
	var island_comp_ok := 0
	for c in comp_sizes.keys():
		if int(comp_sizes[c]) > FeatureTagger.ISLAND_MAX_TILES:
			continue
		island_comp_total += 1
		if int(comp_province_count.get(c, 0)) == 1:
			island_comp_ok += 1
	...
	return {
		...
		"size_ok": min_size >= 1 and max_size <= ProvinceSplitter.MAX_TILES,
	}
